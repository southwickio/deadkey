...
import (
	...
	"encoding/json"  //encoding/decoding json
...
//Scan is one persisted scan run's own record; separate from the per-credential
//AssessmentRow rows it owns. This is what internal/dashboard reads to render a
//past scan's header (when it ran, which providers were checked, which failed
//outright)
type Scan struct {
...
//FinishScan records when scanID completed and which providers (if any) failed
//outright during it. Called once, after every provider has been checked
//
//providersFailed is persisted here; not just returned in-memory to
//cmd/deadkey/scan.go's Report for that one run. It is persisted specifically so
//internal/dashboard can show it when browsing a past scan later. This is the
//kind of schema change the PRAGMA user_version migration system (see
//migrations.go) was built to support safely: providers_failed did not exist in
//the original schema and was added in migration version 2
...
//if scanID doesn't exist, mirroring LatestScanID's not-found convention rather
//than a bare sql.ErrNoRows leaking out to callers
func GetScan(db *DB, scanID int64) (Scan, bool, error) {
	...
	if failedJSON.Valid && failedJSON.String != "" {
		...
		//A decode failure here is deliberately non-fatal. providers_failed is
		//supplementary details, not something that should sink an entire scan's
		//page from rendering
...
//internal/dashboard to render a simple scan-history navigation list; exposing
//history that has been recorded since the very first storage cycle (every scan
//writes new rows, but had no reader for it anywhere until the dashboard needed
//one
