...
//remains a single binary with no runtime file dependencies. The dashboard works
//identically whether it's the binary that was just built here or one someone
//downloaded via curl | sh and ran on an entirely different machine.
...
//no risk of a missing file breaking the dashboard on a machine other than the
//one it was built on
//
...
//specifically for a security tool; and would break the dashboard entirely on an
//offline or air-gapped machine
//
